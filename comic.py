...
# Settings
from settings import SEARCH_QUERIES, RANDOM_VOLUME, SAVE_PATH, API_KEY, BASE_URL, HEADERS
...
def find_volume_id(api_key, query):
    ...
    params = {"api_key": api_key, "format": "json", "query": query, "resources": "volume", "limit": 10 if RANDOM_VOLUME[query] else 1}
...
def get_character_url(api_key, query):
    ...
    if results:
        chosen = results[0]
        print(f"Chosen {chosen['name']} appeared in {chosen['count_of_issue_appearances']} comics")
...
def get_random_character_appearance(api_key, api_detail_url):
    # Our first API call finds a list of volumes that match the search query, and then picks one
    params = {"api_key": api_key, "format": "json", "field_list": "name,volume,issue_credits", "limit": 100}
    response = requests.get(f"{api_detail_url}", headers=HEADERS, params=params)
    response.raise_for_status()
    data = response.json()
    response.close()
    results = data.get("results", {})
    if results:
        ...
        print(f"Chose issue {chosen['name']}")
        return chosen["api_detail_url"]
    else:
        raise ValueError(f"No character url found for {query}.")

def find_random_comic_image(api_key, series_id):
...
def displayComic():
    try:
        ...
        print(f"Chose type {search_type} query {search_query}")
        comic_image_url = None
        image_name = None
        if search_type == "Volume":
            volume_id = find_volume_id(API_KEY, search_query)
            print(f"Found volume id {volume_id}")
            comic_image_url, image_name  = find_random_comic_image(API_KEY, volume_id)
            ...
            comic_image_url, image_name = get_random_comic_image(API_KEY, random_comic_url)
        if (comic_image_url is not None):
            print(f"Would open : {comic_image_url}")
